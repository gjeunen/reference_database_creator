...
After installing any software, most of us will try it out with a help command. Here is an example using crabs on Docker. (All following commands assume that you have already pulled the docker image using `docker pull quay.io/swordfish/crabs:1.14.0.0`.)

```
docker run --rm -it \
  quay.io/swordfish/crabs:1.14.0.0 \
  crabs -h
```
